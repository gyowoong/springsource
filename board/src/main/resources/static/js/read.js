  ...
  fetch(`/replies/board/${bno}`)
    ...
    .then((data) => {
      ...
      data.forEach((reply) => {
        ...
        result += `<div class="p-3"><img src="/img/dafault.png" alt="" class="rounded-circle mx-auto d-block" style="width: 60px; height: 60px" /></div>`;
...
replyForm.addEventListener("submit", (e) => {
  ...
  // 댓글 수정 시 사용
  const rno = replyForm.querySelector("#text");
...
// 이벤트 전파를 이용해서 이벤트 감지(부모 클릭 감지 시)
replyList.addEventListener("click", (e) => {
  ...
  // data-rno 값을 가져오기
  // 수정 버튼 클릭 시 요소의 rno 가져오기
  const rno = btn.closest(".reply-row").dataset.rno;

  // 수정 버튼 or 삭제가 눌러졌는지 구분
  // 클래스명 : classList
